...
export default function Modules() {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-slate-50">
              ...
              {selectedIds.length > 0 && (
                <div className="flex items-center gap-2 bg-primary/10 px-3 sm:px-4 py-2 rounded-lg border border-primary/20">
                  ...
                  <Button
                    ...
                    size="sm"
                    onClick={async () => {
                      if (confirm(`Delete ${selectedIds.length} module(s)?`)) {
                        setActionLoading(true);
                        try {
        ...
        {/* Filters and Search */}
        <div className="bg-white rounded-xl p-4 md:p-5 shadow-sm border border-slate-200 mb-6">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div className="hidden md:flex flex-wrap gap-2 md:gap-3">
              <Select value={levelFilter} onValueChange={setLevelFilter}>
                <SelectTrigger className="bg-slate-50 border-slate-200 rounded-lg px-3 md:px-4 py-2 text-sm w-36 md:w-44 hover:bg-slate-100 transition-colors">
              ...
              <Select value={statusFilter} onValueChange={setStatusFilter}>
                <SelectTrigger className="bg-slate-50 border-slate-200 rounded-lg px-3 md:px-4 py-2 text-sm w-32 md:w-40 hover:bg-slate-100 transition-colors">
              ...
              <Select value={sortBy} onValueChange={setSortBy}>
                <SelectTrigger className="bg-slate-50 border-slate-200 rounded-lg px-3 md:px-4 py-2 text-sm w-44 md:w-52 hover:bg-slate-100 transition-colors">
            ...
            <div className="flex items-center gap-2 w-full lg:w-auto">
              <div className="relative flex-1 lg:flex-initial lg:w-56 xl:w-64">
                <input
                  type="text"
                  placeholder="Search courses..."
                  value={search}
                  onChange={(e) => setSearch(e.target.value)}
                  className="w-full pl-9 md:pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all"
                />
                <Database className="absolute left-2.5 md:left-3 top-1/2 -translate-y-1/2 text-slate-400 w-4 h-4" />
              </div>
              <Button
                variant="outline"
                size="sm"
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button
                    variant="ghost"
                    size="icon"
                    className="text-slate-600 hover:text-primary hover:bg-primary/10 shrink-0"
                  >
                    ...
                    return (
                      ...
                      >
                        ...
                        <input
                          ...
                          className="absolute top-2 left-2 w-4 h-4 text-primary bg-white border-slate-300 rounded focus:ring-primary focus:ring-2 cursor-pointer z-10"
                        />
                ...
                {filtered.map((course: any) => {
                  ...
                  const courseModules = availableModules.filter(
                    (mod) =>
                      ...
                      (mod.courses?.some((c: any) => c.id === course.id) ??
                        false)
                  );
